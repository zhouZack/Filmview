...
-(void)handleTapGesture:(UIGestureRecognizer*)gestureRecognizer{
    ...
    } else if (!CGRectContainsPoint(_scrollView.bounds, locationInScrollView) &&  (locationInScrollView.x < _scrollView.contentOffset.x)) {
#pragma mark - 增加功能:点击左侧区域可以滑动到上一页
        if (_currentPageIndex > 0) {
            _currentPageIndex -= 1;
            [self scrollToPage:_currentPageIndex];
            [self.delegate flowView:self didScrollToPageAtIndex:_currentPageIndex];
        }
    } else if (!CGRectContainsPoint(_scrollView.bounds, locationInScrollView) &&  (locationInScrollView.x > _scrollView.contentOffset.x)) {
#pragma mark - 增加功能:点击右侧区域可以滑动到下一页
        if (_currentPageIndex < [_dataSource numberOfPagesInFlowView:self]) {
            _currentPageIndex += 1;
            [self scrollToPage:_currentPageIndex];
            [self.delegate flowView:self didScrollToPageAtIndex:_currentPageIndex];
        }
